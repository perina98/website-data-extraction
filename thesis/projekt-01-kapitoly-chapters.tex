...
Element \uv{SPAN} označený červenou farbou je náš primárny element. Pri extrakcii teda aplikácia začína červeno označeným elementom \uv{SPAN} a označí otcovský element ako element \uv{P}, ktorý je označený oranžovou farbou. Z daného otcovského elementu prebehne kontrola počtu detských elementov a počtu výsledných objektov. Ich počet je však menší, ako cielený počet. Algoritmus teda zvýši index cieľového otcovského prvku a tým nastaví na cielený otcovský prvok element \uv{DIV} označený zelenou farbou. V tomto prípade je počet priamych detských elementov, nepriamych detských elementov a objektov ktoré ich reprezentujú zhodný s minimálnym požadovaným množstvom. Aplikácia teda našla cielený otcovský element a elementy záujmu.
