...
Pri kontrole výsledkov využíva testovací skript zložky expected a output, kde počet súborov v týchto zložkách obsahujú musí byť zhodný.
...
Nejedná sa totiž len o jednoduché vyobrazenie výsledných dát a porovnanie dát s dátami očakávanými. Táto metrika nám napovie, kde aplikácia dominuje a kde zlyháva. Stále je však potrebné zvážiť aj koreláciu a kauzalitu týchto výsledkov. 
...
Všetky nasledovné časové údaje predstavujú priemernú hodnotu z 5 pokusov, ich hodnota je zaokrúhlená na celé milisekundy a ďalej:
...
Niektoré obsahujú informácií viac, niektoré menej. Niektoré obsahujú irelevantné dáta. Tie je však nutné odfiltrovať. Iné naopak obsahujú len očakávané dáta.
...
Anomálie môžu predstavovať odlišné zaradenie produktov, rôzne reprezentácie ceny a absenciu niektorých vyhľadávaných prvkov(napríklad absencia zľavy niektorých objektov).
...
V tomto prípade sa jedná o sadu zameranú na správy a novinky. Narozdiel od iných sád je v tejto väčšie množstvo textových elementov, medzi ktorými treba rozlišovať. Zároveň sa líšia v reprezentácií časového údaju. Niektoré reprezentujú čas ako dátum, iné v podobe uplynulého času od zverejnenia. 
...
Vykonanie extrakcie na 7 webových dokumentoch priemerne trvalo 8846,4ms, čo nám dáva hodnotu 1769,3ms na jeden dokument. 

Z tejto hodnoty pripadá priemerne 70,43ms na aTime (3,98\%), 8,43ms na eTime (0,48\%) a 1059,71ms na lTime (59,89\%). Zostávajúci čas (35,65\%) pripadal na réžiu aplikácie. Priemerná úspešnosť predstavovala hodnotu 78,31\% a priemerná hodnota recall hodnotu 69,61\%. 

\bigskip

Analýza jednotlivých dokumentov ukázala, že znížené hodnoty precision a recall sú v niektorých prípadoch spojené so spojením jednotlivých prvkov do jedného. Stáva sa tak hlavne pri dvojici \uv{author} a \uv{timestamp}, kde sú tieto dve hodnoty často reprezentované v rámci jedného extrahovaného DOM elementu. Aplikácia následne túto hodnotu určí ako jednu z vyhovujúcich na základe toho, ktorý regulárny výraz predstavuje danú hodnotu.

\newpage

Druhá časť testovacej sady obsahuje 3 webové dokumentu, a z výsledkov vieme vytvoriť nasledovnú tabuľku:
...
Výsledné časové údaje ukázali, že najväčší časový úsek je vyhradený pre načítanie webového dokumentu. Je to však vec, ktorú nieje možné rozumne ovplyvniť. V tomto prípade záleží na obsahu webového dokumentu a jeho veľkosti.

Z časových údajov nevyplynula žiadna výrazná korelácia, avšak jednotlivé hodnoty \texttt{precision} a \texttt{recall} vykazujú na základe Pearsonovej korelácie silnú pozitívnu korelačnú závislosť \cite{statistics}. Túto vieme vypočítať vzorcom \ref{correlation}:
...
Z tohto faktu vieme ďalej vytvoriť predpoklad, že zlepšenie presnosti algoritmu aplikácie povedie k získaniu viacerých relevantných výsledkov. Zlepšenie v tomto zmysle nemusí byť viazané na logiku aplikácie, ale môže byť viazané aj na presnosť definovaného regulárneho výrazu.

\newpage
\section{Paralelizácia načítania webstránok}

Z výsledkov vyplýva aj fakt, že najväčší časový úsek zaberá jednotlivé načítavanie webových dokumentov. Načítavanie dokumentov zaberalo približne 77,14\% celkového času potrebného na extrakciu. Nad rámec zadania bakalárskej práce som implementoval paralelizáciu tohto načítavania. Paralelizácia spočíva v súčasnom načítavaní poskytnutých webových stránok, kde v prípade veľkého množstva stránok je možné načítavanie rozdeliť do skupín tak, aby bolo zaťaženie rozložené.
...
Výsledkom tejto bakalárskej práce je aplikácia určená na inteligentnú extrakciu dát z webových dokumentov. Extrakcia prebieha na základe dopredu definovaných pravidiel zložených z regulárnych výrazov a dátových typov. Hlavným cieľom bolo vytvoriť spôsob extrakcie tak, aby nezáležalo na štruktúre kódu akým je webový dokument reprezentovaný, ale na jeho obsahu. Tento cieľ sa podarilo splniť a vytvoriť tak aplikáciu, ktorá dokáže uľahčiť extrakciu pre bežných používateľov. Zároveň túto extrakciu značne automatizuje a výsledky dosahuje v rozumných časových úsekoch.
...
Aplikácia bola implementovaná v programovacom jazyku Javascript. Využíva moderné technológie ako napríklad Node.js, Puppeteer a Chromium. Vďaka technológii Node.js a jej modernému spôsobu spracovania modulov je navyše inštalácia závislostí jednoduchá a rýchla. Najdôležitejšou technológiou je však pre výslednú aplikáciu práve Puppeteer. Táto technológia dokáže komunikovať s webovým prehliadačom a webovým dokumentom na vysokej úrovni a určite má veľký potenciál. Stretol som sa s ňou v tejto práci síce prvý krát, ale rýchlo som si osvojil jej vlastnosti a funkcie.

Výsledná aplikácia dosahuje výsledky rýchlo a relatívne kvalitne, avšak vždy je miesto na zlepšenie. Jedným z takýchto zlepšení by mohlo napríklad byť vytvorenie vzťahu medzi rôznymi webovými dokumentami a extrahovanými údajmi. Tým by sa mohla extrakcia urýchliť a zároveň zlepšiť presnosť výsledkov. 
